...
int SequentialSearch(List list,KeyType target){
    for(int i = 0; i < list.count; i++){
     if(EQ(list.students[i].stuId,target)){
         return i;
     }   
    }
}
...
int RecBinary2(List list, KeyType target, int bottom, int top) {
  ...
  if (bottom <= top) {
    middle = (top + bottom) / 2;
    if ((target < list.students[middle].stuId)) /* Reduce to the bottom half. */
      middle = RecBinary2(list, target, bottom, middle - 1);
    else if ((target > list.students[middle].stuId)) /* Reduce to the top half. */
...
int Binary2Search(List list, KeyType target) {
  ...
  while (top >= bottom) {
    ...
    middle = (top + bottom) / 2;
    if ((target == list.students[middle].stuId))
      return middle;
    else if ((target < list.students[middle].stuId))
